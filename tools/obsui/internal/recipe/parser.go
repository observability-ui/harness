...
func ParseRecipeArgs(reg *Registry, command string, args []string) ([]RecipeSegment, error) {
	if len(args) == 0 {
		return nil, fmt.Errorf("no recipe specified — run 'obsui list' to see available recipes")
	}
	...
	for _, arg := range args {
		...
		if !ok {
			return nil, fmt.Errorf("unknown recipe %q for command %q — run 'obsui list' to see available recipes", arg, command)
		}
