...
func (processKeyMap) ShortHelp() []key.Binding {
	return []key.Binding{keys.Up, keys.Down, keys.PageUp, keys.PageDown, keys.WrapToggle, keys.Restart, keys.NextTab, keys.PrevTab, keys.Quit}
}

func (processKeyMap) FullHelp() [][]key.Binding {
	return [][]key.Binding{
		{keys.Up, keys.Down, keys.PageUp, keys.PageDown, keys.WrapToggle},
		{keys.Restart, keys.NextTab, keys.PrevTab},
		{keys.Quit},
	}
}
