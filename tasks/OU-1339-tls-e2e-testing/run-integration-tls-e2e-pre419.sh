...
# (Logging, Monitoring, Dashboards, Distributed Tracing)
# simultaneously.
#
# For OCP < 4.19 clusters (TroubleshootingPanel not available).
#
# Usage:
...
readonly PLUGIN_TYPES="Logging Monitoring Dashboards DistributedTracing"
readonly PLUGIN_CR_NAMES="logging monitoring dashboards distributed-tracing"
PLUGIN_DEPLOY_NAMES=""
...
priority_tests() {
  case "$1" in
    ...
    P3) echo "it_10 it_11 it_12 it_12_dt" ;;
...
readonly ALL_TESTS="it_01 it_02 it_03 it_04 it_07 it_10 it_11 it_12 it_12_dt it_05 it_08 it_06 it_09 it_13 it_14 it_15 it_16 it_20 it_17 it_18 it_19"
...
verify_existing_plugins() {
  ...
  if [[ "$missing" == true ]]; then
    log_fail "Missing UIPlugin deployments. Deploy Logging, Monitoring, Dashboards, and DistributedTracing UIPlugins before running this script (OCP < 4.19)."
    exit 1
  fi
...
check_prerequisites() {
  ...
  for cr in $PLUGIN_CR_NAMES; do
    if oc get uiplugin "$cr" &>/dev/null; then
      ...
      log_fail "UIPlugin CR '${cr}' not found — deploy Logging, Monitoring, Dashboards, and DistributedTracing UIPlugins first"
      failed=true
    fi
  done
...
it_03() {
  ...
  if [[ -n "$mon_deploy" ]]; then
    local features_code
    features_code=$(scanner_curl_retry "$(svc_url "$mon_deploy" "/features")")
...
it_11() {
  ...
  local features_code
  features_code=$(scanner_curl_retry "$(svc_url "$deploy" "/features")")
  if [[ "$features_code" == "200" ]]; then
...
COO Integration TLS E2E Tests (OCP < 4.19) — validates TLS across Logging,
Monitoring, Dashboards, and Distributed Tracing UIPlugins simultaneously.
TroubleshootingPanel is excluded (not available before OCP 4.19).
...
main() {
  ...
  log_info "Plugins:   Logging, Monitoring, Dashboards, Distributed Tracing"
